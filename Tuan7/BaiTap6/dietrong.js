...
function initStats() {
  ...
  gameContinue = true;
  document.getElementById("statusText").innerText = "Nhấn \"RA ĐÒN!\" để bắt đầu hiệp đấu.";
...
function updateUI() {
  document.getElementById("heroStats").innerText = "STR " + strength + " | AGI " + agility + " | INT " + intel;
  document.getElementById("dragonStats").innerText = "STR " + dragonStr + " | AGI " + dragonAgi + " | INT " + dragonInt;

  document.getElementById("heroHpText").innerText = "Máu: " + Math.max(0, health) + "/" + maxHeroHp;
  document.getElementById("dragonHpText").innerText = "Máu: " + Math.max(0, dragonHealth) + "/" + maxDragonHp;
...
function playTurn() {
  ...
  if (heroHit) {
    ...
    logHero("XÍ HỤT! Rồng đã tránh được đòn tấn công của bạn.");
  }
  ...
  if (health <= 0) {
    ...
    gameContinue = false;
    document.getElementById("btnTurn").disabled = true;
    logSystem("THẤT BẠI! Anh hùng đã bị Rồng hạ gục.");
    alert("THẤT BẠI!!!");
    document.getElementById("statusText").innerText = "Bạn đã thua trận...";
    return;
  }

  document.getElementById("statusText").innerText = "Hiệp đấu kết thúc. Nhấn \"RA ĐÒN!\" để tiếp tục.";
}
